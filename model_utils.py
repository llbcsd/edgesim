...
class BertCustomEmbeddings(nn.Module):
    ...
    def __init__(self, config, custom_config):
        ...
        self.arch_type_embeddings = nn.Embedding(custom_config['arch_num'], config.hidden_size)
    ...
    def forward(
        ...
        arch_ids: Optional[torch.LongTensor] = None,
        ...
        # add custom arch 
        arch_embedding = self.arch_type_embeddings(arch_ids)
        embeddings += arch_embedding
...
class BertCustomModel(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, custom_config=None):
        super().__init__(config)
        self.config = config

        self.embeddings = BertCustomEmbeddings(config, custom_config)
...
class BlockRelatePredict(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, relate_size):
        super().__init__()
...
class BertOnlyCTPHead(nn.Module):
...
class BertForMLMAndCTP(BertPreTrainedModel):
    ...
    def __init__(self, config, rela_size):
        ...
        self.octp = BertOnlyCTPHead(hidden_size=config.hidden_size, relate_size=rela_size)
    ...
    def forward(
        ...
        seq_relationship_scores = self.octp(pooled_output)
...
@dataclass
class MLMBRPOutput(ModelOutput):
    """
    ...
    brp_logits: torch.FloatTensor = None
